...
int main() {
    ...
    */
    ...
    for (int j = 0; j <= n_plot; j++) {
        q = j * step_plot;
        q2 = q * q;
        x2 = (c / sqrt(16.L * M_PI * M_PI * c * c * om * om - q2 * q2)) * (M_PI + 2.L * atan((8.L * M_PI * M_PI * c * c * om * om - q2 * q2) / (q2 * std::sqrt(16.L * M_PI * M_PI * c * c * om * om - q2 * q2))));

        out << q << ", " << setprecision(20) << real(x2) << endl;
    }
    out.close();
